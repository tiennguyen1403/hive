...
describe("supabase/seed.sql", () => {
  ...
  it("ends by rebuilding the live tables from the mirrors, on the real clock's anchor", () => {
    expect(onDisk.trimEnd().endsWith("select public.reset_demo(public.demo_anchor());")).toBe(true);
  });
});
