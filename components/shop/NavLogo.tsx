/**
 * The HIVE lockup as the nav bar wears it: the mark and the wordmark, inline.
 *
 * Copied from `prototype/name/logo/hive-lockup-nav.svg` (mark M2 + wordmark W3,
 * QĐ-30). That file draws the letters at 62.5% of the mark's height, and the
 * proportion belongs to the bar alone: at 30–32px the narrow stencil letters
 * read smaller than the mark at the logo sheet's own 52%, so the user took
 * them to 20px on a 32px mark and 18.8px on a 30px one (24/09/2026). The
 * logo files keep 52%.
 *
 * The colours are the logo's own rather than tokens: a logo does not follow
 * a theme.
 *
 * `aria-hidden` and no `<title>`: the link around it is already named
 * "HIVE, trang chủ", and a second name inside it would be read out twice.
 * The size is set by `.s .nav3 .wm svg` in nav.css and desktop.css: 30px tall
 * on the phone, 32px from 900px, the width following the viewBox.
 */
export function NavLogo() {
  return (
    <svg viewBox="-500 -500 2364.99 1000" aria-hidden="true">
      ...
      <path
        fill="#171410"
        d="M860.7,312.5h-84.38v-625h84.38v271.09h67.58l-19.92,75h-47.66zM1036.09,312.5h-84.38v-625h84.38zM1208.74,312.5h-84.38v-625h84.38zM1340.77,210.94l-66.41,-523.44h87.5l39.84,330.85l13.28,192.57zM1356.79,312.5l-10.55,-75h89.45l13.68,-219.13l39.46,-330.85h87.9l-80.47,625zM1726.31,312.5h-84.38v-625h84.38zM1864.99,312.5h-102.73v-75h102.73zM1857.18,33.59h-94.92v-75h94.92zM1864.99,-237.5h-102.73v-75h102.73z"
      />
    </svg>
  );
}
