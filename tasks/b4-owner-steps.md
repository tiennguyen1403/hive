...
- [ ] **A1 Tài khoản.** supabase.com → đăng nhập bằng GitHub. Gói Free, không cần thẻ.
- [ ] **A2 Dự án.** New project → Organization: tạo mới nếu chưa có (Free) → Name: `hive-demo`
...
- [ ] **A3 Hai khoá → `.env.hosted.local`.** Project Settings (bánh răng) → *API Keys*:
...
- [ ] **A4 Tắt xác nhận email.** Authentication → *Sign In / Providers* (bản cũ: *Providers*) →
...
- [ ] **A5 Hai lệnh, trong PowerShell của bạn tại `D:\Code\e-commerce`** (không qua tôi, vì
      cần gõ mật khẩu):
...
- [ ] **Báo tôi một câu.** Tôi đọc mã dự án từ `supabase/.temp/project-ref`.
...
## C · Vercel — khoảng 10 phút, làm sau khi tôi báo đã đẩy schema và code B4
...
- [ ] **C3 Màn cấu hình.** Project Name: `hive-demo` (địa chỉ sẽ là `hive-demo.vercel.app`,
      ...
      `CRON_SECRET`. Kiểm đủ 5, không dòng nào còn chữ "dan-…-vao-day". Environment: để mặc
...
1. `npx supabase db push` (6 migration), seed qua `reset_demo`, `npm run seed:users` trỏ vào dự
   án hosted, kiểm bucket `product-photos` và `catalog_snapshot` từ máy này.
