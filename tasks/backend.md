...
| **B3 Quản trị** | Claim admin; 16 hành động thành Server Actions (guard chuyển trạng thái trong SQL); `events` thay `brand.adminSim`; "Đặt lại dữ liệu mẫu" gọi `reset_demo()`; đổi lời simbar; đồng hồ thật, anchor = `now()` | mọi nút admin ghi DB; log/CSV từ `events`; reset đưa về seed |
| **B4 Triển khai** | Dự án Supabase hosted (`ap-southeast-1`), `supabase db push`, seed; Vercel (Singapore) + cron; GitHub public | link công khai chạy; cron thấy trong log |
